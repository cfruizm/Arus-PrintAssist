...
try:
 enabled=bool(st.secrets.get("AGENT_CORE_LAB_ENABLED",False));batch=int(st.secrets.get("AGENT_CORE_LAB_BATCH_SIZE",3));tokens=int(st.secrets.get("LLM_ORCHESTRATOR_MAX_TOKENS",180));policy=str(st.secrets.get("LLM_INTERNAL_KNOWLEDGE_POLICY","hybrid_guarded"));real_shadow=bool(st.secrets.get("AGENT_CORE_SEMANTIC_REAL_CHAT_SHADOW_ENABLED",False));answer_tokens=int(st.secrets.get("LLM_ANSWER_MAX_TOKENS",700))
except Exception:enabled=False;batch=3;tokens=180;policy="hybrid_guarded";real_shadow=False;answer_tokens=700
...
with evidence_tab:
 st.write("Política activa:",policy);items=json.loads(Path("tools/agent_core_lab/evidence_policy_cases.json").read_text(encoding="utf-8"));sid=st.selectbox("Escenario documental",[x["id"] for x in items]);item=next(x for x in items if x["id"]==sid);decision=evaluate_document_support(item["metrics"],policy,item["intent"]);st.json({"input":item,"decision":decision.to_dict(),"response_contract":build_response_contract(decision),"passed":decision.response_mode==item["expected"]})
with answer_tab:
 st.warning("Laboratorio aislado. Ejecuta retrieval real y una respuesta Groq, pero no modifica el chat.")
 query=st.text_area("Consulta técnica",value="Los trabajos dejan de verse después del envío en PaperCut MF. ¿Qué validación sigue?",height=90)
 product=st.text_input("Producto conocido",value="PaperCut MF");symptom=st.text_input("Síntoma confirmado",value="Los trabajos dejan de verse después del envío");failed=st.text_input("Acción fallida",value="Reinicio del proveedor de impresión")
 if st.button("1. Recuperar evidencia",type="primary"):
  st.session_state.hybrid_retrieval=retrieve_from_existing_backend(query,6)
 if "hybrid_retrieval" in st.session_state:st.json(st.session_state.hybrid_retrieval)
 can_answer=bool(st.session_state.get("hybrid_retrieval",{}).get("ok"))
 if st.button("2. Evaluar y generar respuesta",disabled=not can_answer):
  state={"products":[product] if product else [],"symptoms":[symptom] if symptom else [],"failed_actions":[failed] if failed else []};config=load_gateway_config(st.secrets);gateway=LLMGateway(config,st.session_state);st.session_state.hybrid_answer=run_hybrid_response_lab(gateway,st.session_state.hybrid_retrieval,query,state,"troubleshooting",policy,max(200,min(1200,answer_tokens)))
 if "hybrid_answer" in st.session_state:
  result=st.session_state.hybrid_answer;st.json(result);text=result.get("answer_result",{}).get("text","");
  if text:st.markdown(text)
  st.download_button("Descargar respuesta híbrida",json.dumps(result,ensure_ascii=False,indent=2),file_name="agent_core_hybrid_response_lab.json",mime="application/json")
with real_tab:
 st.write("**Flag activo:**",real_shadow);records=list(st.session_state.get("agent_core_semantic_real_chat_records",[]) or []);st.json({"summary":summarize_records(records),"records":records})
