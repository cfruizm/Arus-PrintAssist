...
st.caption("Laboratorio consolidado: orquestación semántica y política de evidencia híbrida.")
try:enabled=bool(st.secrets.get("AGENT_CORE_LAB_ENABLED",False));batch=int(st.secrets.get("AGENT_CORE_LAB_BATCH_SIZE",3));tokens=int(st.secrets.get("LLM_ORCHESTRATOR_MAX_TOKENS",220));policy=str(st.secrets.get("LLM_INTERNAL_KNOWLEDGE_POLICY","hybrid_guarded"))
except Exception:enabled=False;batch=3;tokens=220;policy="hybrid_guarded"
...
with sem_tab:
 ...
 if st.session_state.agent_core_lab_records:
  summary=summarize(st.session_state.agent_core_lab_records);st.json({"summary":summary,"records":st.session_state.agent_core_lab_records});st.download_button("Descargar semántica",json.dumps({"summary":summary,"records":st.session_state.agent_core_lab_records},ensure_ascii=False,indent=2),file_name="agent_core_semantic_results.json",mime="application/json")
with evidence_tab:
