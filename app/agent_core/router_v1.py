...
from app.agent_core.router_normalizer import extract_urls,normalize_text
...
INTAKE=(r"^necesito ayuda$",r"^necesito soporte$",r"^tengo problemas?$",r"^algo no funciona$")
...
def route_message(message: str,state: RouterShadowState) -> RouteDecision:
    raw=str(message or "").strip(); text=normalize_text(raw); entities=detect_entities(raw); products=[e["canonical_name"] for e in entities["products"]]; processes=[e["canonical_name"] for e in entities["processes"]]
    if text in SOCIAL: decision=RouteDecision("social","social_message",1.0)
    elif extract_urls(raw): decision=RouteDecision("explicit_source","explicit_url",1.0,use_retrieval=True,use_llm=True,metadata={"urls":extract_urls(raw)})
    ...
    elif text in {"como asi","no entiendo","explicame mejor"}:
        decision=RouteDecision("clarification","request_to_explain_previous_response",0.9,inherit_context=bool(state.conversation.last_route))
    elif products or processes:
        decision=RouteDecision("technical_query","explicit_registered_entity",0.98,use_retrieval=True,use_llm=True)
    ...
    decision.deterministic_response=build_response(decision,state)
    state.conversation.last_route=decision.route;state.turn_number+=1
    return decision
