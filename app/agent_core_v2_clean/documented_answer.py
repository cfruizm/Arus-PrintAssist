...
PROMPT_VERSION="conceptual_documented_v2_quality_preserved"
...
class DocumentedAnswerComposer:
 def __init__(self,gateway,max_tokens=260):self.gateway=gateway;self.max_tokens=max(220,min(420,int(max_tokens)));self.last_provider_result={}
 def compose(self,message,understanding,retrieval):
  ...
  text=str(r.text or "").strip();valid,cited=validate_citations(text,[str(x["id"]) for x in evidence])
