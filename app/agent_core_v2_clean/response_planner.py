...
def build_response_plan(message,u,retrieval,decision=None):
 ...
 rp={'mode':mode,'must_answer_general':not documented,'must_ask_one_detail':bool(missing),'question_target':missing[0] if missing else None,'allow_documented_claims':documented,'allow_internal_guidance':not documented or mode=='hybrid','allow_example_claims':example,'complete_goal':documented and decision.get('status')=='sufficient' and not missing};return ResponsePlan(1,request,ep,rp)
