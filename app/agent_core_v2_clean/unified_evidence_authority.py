...
def apply_unified_evidence_verdict(retrieval,message,understanding):
 ...
 intent=str((understanding or {}).get("intent") or "");scored.sort(key=lambda p:((p[0]["definition_match"] if intent=="conceptual" else p[0]["operation_match"]),p[0]["title_hit_count"],p[0]["covered_count"],p[0]["semantic_score"]),reverse=True)
 ...
 out["evidence_verdict"]=verdict;out["generation_evidence"]=deepcopy(selected);out["evidence"]=deepcopy(selected);sf=out.setdefault("semantic_fit",{});sf.update({"accepted_for_generation":accepted,"low_fit":not accepted,"generation_ids":verdict["evidence_ids"],"generation_count":len(selected),"selected_document":verdict["document_ids"][0] if verdict["document_ids"] else None,"decision_path":"unified_evidence_authority_v6_subject_granularity_token_control","canonical_status":status});return out
