...
def _fit(item,wanted):
 title=_tokens(item.get("title"));text_tokens=_tokens(item.get("text"));content=title|text_tokens;covered=wanted&content;hits=wanted&title;sem=float((item.get("semantic_fit") or {}).get("score",0) or 0);operations=wanted&_OPERATION;operation_hits=operations&content
 body=_norm(item.get("text"))[:220];subject=sorted(wanted-_OPERATION,key=lambda x:(-len(x),x));definition_match=bool(subject and all(x in body for x in subject) and re.search(r"\b(is|are|es|son|permite|provides?)\b",body))
 return {"coverage":len(covered)/max(1,len(wanted)),"title_coverage":len(hits)/max(1,len(wanted)),"covered_count":len(covered),"title_hit_count":len(hits),"semantic_score":sem,"operation_match":bool(not operations or operation_hits),"operation_hits":sorted(operation_hits),"definition_match":definition_match,"covered":sorted(covered),"title_hits":sorted(hits)}
def apply_unified_evidence_verdict(retrieval,message,understanding):
 ...
 scored.sort(key=lambda p:(p[0]["definition_match"],p[0]["operation_match"],p[0]["title_hit_count"],p[0]["covered_count"],p[0]["title_coverage"],p[0]["coverage"],p[0]["semantic_score"]),reverse=True)
 ...
 if best and wanted:
  ...
  operation_ok=bool(best_fit.get("operation_match"))
  exact_document=title_direct and target_ratio>=.34 and short_ok and operation_ok
  relevant_answerable=short_ok and operation_ok and target_ratio>=.25 and (title_direct or (content_direct and semantic_support))
 ...
 out["evidence_verdict"]=verdict;out["generation_evidence"]=deepcopy(selected);out["evidence"]=deepcopy(selected);sf=out.setdefault("semantic_fit",{});sf.update({"accepted_for_generation":accepted,"low_fit":not accepted,"generation_ids":verdict["evidence_ids"],"generation_count":len(selected),"selected_document":verdict["document_ids"][0] if verdict["document_ids"] else None,"decision_path":"unified_evidence_authority_v5_operation_entailment","canonical_status":status});return out
