...
def _generate_documented(result,message,secrets,s,budget,store):
 ...
 if cached:
  result["answer"]=deepcopy(cached["answer"]);result["answer"].setdefault("documented_evidence_used",True);result["answer"].setdefault("internal_knowledge_used",False);result["answer"].setdefault("knowledge_mode","documented_only");result["documented_answer"]={**deepcopy(cached["diagnostic"]),"cache_hit":True};store["cache_metrics"]["documented_answer_hits"]+=1;return result,{"skipped":True,"reason":"documented_answer_cache"}
...
def process_message(message,secrets,s):
 ...
 if cached:
  store["memory"].turn_number+=1;result={"input":message,"state_before":before,**deepcopy(cached["artifact"]),"state_after":deepcopy(store["memory"].to_dict()),"provider_trace":{"understanding":{"skipped":True,"reason":"exact_turn_cache"},"response":{"skipped":True,"reason":"exact_turn_cache"}},"execution":{**execution,"cache_hit":True},"cache":{"hit":True,"type":"exact_turn"},"production_changed":False};result,doc_trace=_generate_documented(result,message,secrets,s,budget,store);result["provider_trace"]["documented_answer"]=doc_trace or {"skipped":True,"reason":"documented_answer_not_called"}
  if doc_trace and not doc_trace.get("skipped"):add_result(store["telemetry"],doc_trace)
  result["turn_metrics"]={"calls":0,"prompt_tokens":0,"completion_tokens":0,"total_tokens":0,"provider_failed_calls":0,"contract_failed_calls":0,"functional_failed_calls":0} if (doc_trace or {}).get("skipped") else turn_metrics(None,doc_trace)
  result["session_metrics_after_turn"]=snapshot(store["telemetry"]);text=result["answer"]["text"];store["cache_metrics"]["hits"]+=1;store["cache_metrics"]["calls_avoided"]+=1;store["messages"] += [{"role":"user","content":message},{"role":"assistant","content":text}];store["turns"].append(result);return result
