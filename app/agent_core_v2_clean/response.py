...
from .context_constraints import build_confirmed_case_context,confirmed_case_prompt
SYSTEM="""Natural enterprise printing-support colleague. Use the user's language. Help first. Answer immediately when sufficiently clear. Ask exactly one short targeted question only when one missing fact is indispensable and materially changes the answer. If asking, do not include examples, technology lists, checklists, speculative mechanisms or a second question. Add at most one short sentence explaining why the answer matters. Retrieval is disabled here: never provide exact menus, service names, commands, paths, values or product procedures. Diagnostic hypotheses are possibilities, never confirmed causes or exclusions unless the user supplied confirming evidence. Concise."""
class NaturalResponseComposer:
 def __init__(self,gateway,max_tokens=220):self.gateway=gateway;self.max_tokens=max(120,min(300,int(max_tokens)));self.last_provider_result={};self.context_diagnostic={}
 def compose(self,message,m,u,d):
  ...
  self.context_diagnostic={"applied":True,**{k+"_count":len(v) for k,v in build_confirmed_case_context(m).items() if isinstance(v,list)}}
  from app.llm_gateway.models import LLMRequest
  r=self.gateway.complete(LLMRequest([{"role":"system","content":SYSTEM},{"role":"user","content":json.dumps({"message":message,"memory":compact_context(m),"understanding":u.to_dict(),"decision":d.to_dict(),"confirmed_case_context":build_confirmed_case_context(m),"confirmed_case_constraints":confirmed_case_prompt(m)},ensure_ascii=False,separators=(",",":"))}],"agent_core_v2_clean_response",min(self.max_tokens,150),0.,None));self.last_provider_result=r.to_dict()
