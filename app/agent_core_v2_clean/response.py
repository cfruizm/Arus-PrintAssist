...
from .case_context import build_case_context,prompt_constraints
...
class NaturalResponseComposer:
 ...
 def compose(self,message,m,u,d):
  ...
  r=self.gateway.complete(LLMRequest([{"role":"system","content":SYSTEM},{"role":"user","content":json.dumps({"message":message,"memory":compact_context(m),"understanding":u.to_dict(),"decision":d.to_dict(),"confirmed_case_context":build_case_context(m),"confirmed_case_constraints":prompt_constraints(m)},ensure_ascii=False,separators=(",",":"))}],"agent_core_v2_clean_response",min(self.max_tokens,150),0.,None));self.last_provider_result=r.to_dict()
