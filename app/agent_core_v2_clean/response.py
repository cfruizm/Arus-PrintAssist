...
class NaturalResponseComposer:
 ...
 def compose(self,message,m,u,d):
  ...
  goal=str(getattr(u,"current_goal","") or "").casefold();msg=str(message or "").strip().casefold()
  if intent in {"meta","capabilities"} or act=="request_capabilities":
   return AgentResponse("Puedo explicar conceptos y productos de impresión, consultar procedimientos y requisitos respaldados por la documentación, orientar diagnósticos, comparar alternativas y preparar información para escalamiento. Cuando la documentación no sea suficiente, separo claramente la orientación general.","capabilities")
  if intent=="social" or act in {"social","acknowledgement"}:
   if act=="acknowledgement" or any(x in goal for x in ("agradec","thanks","thank")) or msg in {"gracias","muchas gracias","thanks","thank you"}:return AgentResponse("Con gusto. Si necesitas revisar otro caso de impresión, aquí estoy.","social")
   if intent=="cancel" or act=="cancel" or any(x in goal for x in ("cerrar","finalizar","terminar","close")):return AgentResponse("Entendido. Doy por finalizada la conversación.","cancelled")
