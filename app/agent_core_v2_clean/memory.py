...
def apply_understanding(m,u):
 if u.topic_relation=="new_topic" and u.domain_relevance!="out_of_scope" and m.active_topic and u.current_goal!=m.active_topic:
 ...
 if u.topic_relation!="independent" and u.domain_relevance!="out_of_scope":
  ...
  for f in u.case_updates:
   ...
   if k in {"symptom","reported_failure"}:_add(m.support_case.symptoms,v);m.support_case.status="diagnosing"
