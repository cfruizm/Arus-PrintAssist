...
def finalize_citations(text,plan):
 plan=plan or {};ep=plan.get('evidence_plan') or {};rp=plan.get('response_plan') or {};mapping={str(k):str(v) for k,v in (ep.get('citation_map') or {}).items()};valid={str(x) for x in ep.get('documented_ids') or []};source=str(text or '');remapped={};preserved=[]
 def resolve(cid):
...
def enforce_answer_contract(payload,canonical_plan):
 result=dict(payload or {});text,a=finalize_citations(result.get('text',''),canonical_plan);result['text']=text;allow=bool(((canonical_plan or {}).get('response_plan') or {}).get('allow_documented_claims'));internal=bool(result.get('internal_knowledge_used'));result['documented_evidence_used']=allow and bool(a.cited_ids) and a.valid
