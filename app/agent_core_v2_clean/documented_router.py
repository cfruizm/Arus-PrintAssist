...
def _internal(result,message,gateway,budget,store,model,assessment):
 ...
 c=ControlledInternalKnowledgeComposer(gateway,520);a=c.compose(message,u,r,assessment);valid=a.mode in {'controlled_internal_knowledge','controlled_internal_knowledge_partial'};payload=a.to_dict();payload['text']=normalize_citation_groups(payload.get('text',''));payload.update(_flags(c.validation,valid));result['answer']=payload;diag={'enabled':True,'cache_hit':False,'prompt_version':INTERNAL_PROMPT_VERSION,'trigger_status':assessment.get('status'),'trigger_reasons':assessment.get('reasons'),'validation':deepcopy(c.validation),'canonical_plan_used':True};result['internal_knowledge']=diag
