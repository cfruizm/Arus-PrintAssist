...
def _internal(result,message,gateway,budget,store,model,assessment):
 ...
 composer=ControlledInternalKnowledgeComposer(gateway,520);answer=composer.compose(message,u,r,assessment);payload=answer.to_dict();valid=answer.mode=="controlled_internal_knowledge";documented_used=bool(composer.validation.get("documented_citations"));payload.update({"documented_evidence_used":documented_used,"internal_knowledge_used":valid,"knowledge_mode":"documented_plus_internal" if valid and documented_used else "internal_only" if valid else "documented_only" if documented_used else "none"});result["answer"]=payload;diag={"enabled":True,"cache_hit":False,"prompt_version":INTERNAL_PROMPT_VERSION,"trigger_status":assessment.get("status"),"trigger_reasons":assessment.get("reasons"),"validation":deepcopy(composer.validation)};result["internal_knowledge"]=diag
...
def maybe_generate_procedural(result,message,gateway,budget,store,model=""):
 ...
 if u.get("intent") not in {"procedural","troubleshooting"}:return result,None
