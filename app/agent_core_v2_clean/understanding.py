...
class ConversationUnderstanding:
 def __init__(self,gateway,max_tokens=180):self.gateway=gateway;self.max_tokens=max(120,min(220,int(max_tokens)));self.last_provider_result={}
 def _degraded(self,message,memory,reason):
  active=memory.pending_goal.summary or memory.active_topic or "";is_answer=bool(memory.last_assistant_question)
  # Provider failure must never reinterpret or mutate topic/case. The raw message remains in the turn log for replay.
  return TurnUnderstanding("answer_to_question" if is_answer else "follow_up",memory.pending_goal.intent or "unknown","same_topic","uncertain",active,False,{},[],False,None,False,.0,reason,True)
