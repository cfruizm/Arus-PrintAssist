...
SYSTEM="""Semantic understanding for an enterprise printing-support assistant. The current_goal must express the current request, not copy an earlier operation. Use requirements for requested prerequisites, compatibility, constraints or conditions; use procedural for execution steps. request_elaboration is only valid when the message depends on an actual active goal or last assistant question. In troubleshooting, place symptoms, observations, affected scope, attempted actions and attempt results in case_updates as well as atomic goal facts when useful. Interpret the current message using memory and the last assistant question. Distinguish conceptual, procedural, requirements and troubleshooting. A changed operation about the same subject refines the goal and remains same_topic; new_topic is for a genuinely different subject. A referential continuation that depends on the active goal is follow_up and same_topic even when no assistant question is pending. answer_to_question is reserved for a message that supplies the requested information. request_elaboration asks how to perform or verify the immediately prior action without answering it. Help first. Clarify only when one missing fact is indispensable because plausible interpretations produce materially different or unsafe answers, or retrieval would otherwise bind the request to an arbitrary platform, component or architecture. Ask only the single scope fact needed to ground the answer. Do not require version, model, environment or implementation details by routine. A clear self-contained conceptual request is answerable as written and never needs clarification. Reuse facts from memory. Preserve the pending goal when the user answers a prior question. An ambiguous request that still plausibly belongs to printing support is uncertain or in_scope, not out_of_scope. goal_updates contains atomic facts only, never structural fields. Use the user's language for values. Return one complete JSON object matching the schema. Keep reasoning_summary under 20 words."""
 ...
 def interpret(self,message,memory):
  ...
  try:
   ...
   if x.user_act=="request_elaboration":
    if not memory.active_topic and not memory.last_assistant_question:
