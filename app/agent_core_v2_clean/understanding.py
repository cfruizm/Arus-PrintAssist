...
from .model_profile import profile_for,is_quota_error,is_json_validation_error
...
class ConversationUnderstanding:
 ...
 def interpret(self,message,memory):
  ...
  def req(payload,purpose,force_text=False):
   text_mode=force_text or self.profile.understanding_mode=="text_json"
   system=SYSTEM + (" Return one raw JSON object only; do not use Markdown fences or commentary." if text_mode else "")
   return self.gateway.complete(LLMRequest([{"role":"system","content":system},{"role":"user","content":json.dumps(payload,ensure_ascii=False,separators=(",",":"))}],purpose,self.max_tokens,0.,None if text_mode else UNDERSTANDING_SCHEMA))
  base_payload={"message":message,"context":compact_context(memory)}
  r=req(base_payload,"agent_core_v2_clean_understanding");self.last_provider_result=r.to_dict();self.contract_valid=False;self.validation_error=None;self.normalization={"removed_goal_update_keys":[],"model_profile":self.profile.to_dict()}
  if not r.ok and is_json_validation_error(r) and not is_quota_error(r):
   text_retry=req({**base_payload,"instruction":"Return exactly one complete JSON object matching the requested fields."},"agent_core_v2_clean_understanding_text_recovery",True)
   self.last_provider_result={"initial":r.to_dict(),"text_recovery":text_retry.to_dict(),"text_recovery_attempted":True}
   self.normalization["json_validation_text_recovery_attempted"]=True
   if text_retry.ok:
    try:
     x=self._parse(text_retry.text);self.contract_valid=True;self.normalization["json_validation_text_recovery_succeeded"]=True;return self._normalize(x,memory)
    except Exception as recovery_exc:self.normalization["json_validation_text_recovery_error"]=str(recovery_exc)
   r=text_retry
  if not r.ok:
   self.validation_error="provider_error:"+str(r.error_code or "unknown");self.normalization["quota_limited"]=is_quota_error(r);return self._degraded_current(message,self.validation_error)
