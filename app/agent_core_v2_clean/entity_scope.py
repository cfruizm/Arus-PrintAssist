...
ALIASES={"manufacturer":{"manufacturer","brand","vendor","fabricante","marca"},"product":{"product","platform","solution","producto","plataforma","solucion"},"model":{"model","device_model","printer_model","modelo"},"operating_system":{"operating_system","os","sistema_operativo"},"architecture":{"architecture","arch","arquitectura"}}
...
def normalize_scope(details=None):
 raw={str(k).casefold():str(v).strip() for k,v in (details or {}).items() if str(v).strip()};values={}
