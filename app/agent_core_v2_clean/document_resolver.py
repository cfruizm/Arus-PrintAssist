...
def _catalog(force=False):
 ...
 try:
  ...
  raw=col.get(include=["metadatas"]);metas=raw.get("metadatas") or []
  unique={}
  for m in metas:
 ...
 return _CACHE
def resolve(*values):
 ids=identifiers(*values);cat=_catalog();matches=[]
 for e in cat["entries"]:
  if ids and any(i in e["normalized"] for i in ids):matches.append(e)
...
def retrieve(resolution,query,k=24):
 ...
 source=resolution["matches"][0]["source"]
 try:
  from app.integration.document_expansion_adapter import retrieve_same_document
  raw=retrieve_same_document(query,source,k) or {};return raw.get("evidence") or []
