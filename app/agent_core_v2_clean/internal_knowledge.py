...
class ControlledInternalKnowledgeComposer:
    ...
    def compose(self, message, understanding, retrieval, assessment):
        ...
        if not evidence:
            value=str(result.text or "").strip();complete=str(result.finish_reason or "").casefold() not in {"length","max_tokens"};valid=bool(value) and complete and not re.search(r"\[(R\d+)\]",value);diagnostic={"safe_partial":bool(value),"sections_present":[],"separation_valid":True,"finish_complete":complete,"documented_citations":[],"internal_citations":[],"unknown_citations":[]}
        else:
        ...
        return AgentResponse((f"{WARNING}\n\n" if evidence else "")+str(result.text).strip(),"controlled_internal_knowledge",True,result.provider,result.model,usage,result.finish_reason)
