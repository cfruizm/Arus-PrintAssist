...
class CleanConversationalAgent:
 ...
 def process(self,message,memory):
  before=deepcopy(memory.to_dict());u=self.understanding.interpret(message,memory);u,entity_frame=reconcile_entity_frame(memory,u);u,c,b=reconcile_turn(u,memory,message);u,e=reconcile_understanding_object(u,memory,b);n=deepcopy(self.understanding.normalization or {});n.setdefault("structural_corrections",[]);n["structural_corrections"].extend(x for x in c+[z.get("reason") for z in e] if x and x not in n["structural_corrections"]);d=self.policy.decide(u,memory);apply_understanding(memory,u);apply_reopen_transition(memory,e);a=self.response.compose(message,memory,u,d);q=_closing_question(a.text)
