...
class CleanConversationalAgent:
 ...
 def process(self,message,memory):
  ...
  if scope_boundary.blocked_before_retrieval:
   ...
   return {"input":message,"state_before":before,"understanding":u.to_dict(),"understanding_contract":{"valid":self.understanding.contract_valid,"error":self.understanding.validation_error},"decision":d.to_dict(),"state_after":deepcopy(memory.to_dict()),"answer":a.to_dict(),"provider_trace":{"understanding":self.understanding.last_provider_result,"response":self.response.last_provider_result},"retrieval":{"enabled":False,"skipped_reason":"domain_boundary_pre_retrieval"},"scope_boundary":scope_boundary.to_dict(),"functional_events":[{"type":"out_of_scope_blocked_before_retrieval"}],"production_changed":False}
