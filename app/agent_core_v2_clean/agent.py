...
class CleanConversationalAgent:
 ...
 def process(self,message,memory):
  before=deepcopy(memory.to_dict());u=self.understanding.interpret(message,memory);d=self.policy.decide(u,memory);apply_understanding(memory,u);a=self.response.compose(message,memory,u,d);asked=_closing_question(a.text)
  if asked:
