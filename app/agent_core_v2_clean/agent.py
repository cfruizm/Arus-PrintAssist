...
def _pending_prompt(text):
 value=" ".join(str(text or "").split()); found=list(re.finditer(r"¿[^?]{1,420}\?",value));
 if found:return found[-1].group(0).strip()
 for sentence in reversed(re.split(r"(?<=[.!])\s+",value)):
  if re.match(r"^(verifica|confirma|indica|revisa|comprueba)\b",sentence,re.I):return sentence.strip()
 return None
...
class CleanConversationalAgent:
 ...
 def process(self,message,memory):
  before=deepcopy(memory.to_dict());u=self.understanding.interpret(message,memory);d=self.policy.decide(u,memory);apply_understanding(memory,u);a=self.response.compose(message,memory,u,d);asked=_pending_prompt(a.text)
  if asked:
