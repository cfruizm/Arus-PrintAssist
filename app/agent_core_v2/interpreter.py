...
def _clean_message_as_symptom(message):
 text=re.sub(r"\s+"," ",str(message or "")).strip()
 text=re.sub(r"(?i)\s*[¿?]*(qué|que|cuál|cual|cómo|como)\s+debo\s+(validar|hacer).*$","",text).strip(" .¿?")
 return text[:300]
def normalize_payload(raw,message=""):
 ...
 confidence=float(raw.get("confidence",0));entities=[]
 for x in raw.get("entities") or []:
  if not isinstance(x,dict):continue
  name=str(x.get("canonical_name") or x.get("name") or x.get("value") or "").strip()
  if name:entities.append({"kind":str(x.get("kind") or x.get("type") or "product"),"canonical_id":str(x.get("canonical_id") or x.get("id") or _id(name)),"canonical_name":name,"matched_text":str(x.get("matched_text") or x.get("mention") or name),"confidence":float(x.get("confidence",confidence or .7))})
 raw["entities"]=entities;facts=[]
 for x in raw.get("facts") or []:
  if isinstance(x,str):facts.append({"type":"symptom","value":x,"confidence":confidence or .7})
  elif isinstance(x,dict) and str(x.get("value") or "").strip():facts.append({"type":str(x.get("type") or "technical_context"),"value":str(x.get("value")),"confidence":float(x.get("confidence",confidence or .7))})
 if raw["intent"]=="troubleshooting" and not any(x.get("type")=="symptom" for x in facts):
  symptom=_clean_message_as_symptom(message)
  if symptom:facts.append({"type":"symptom","value":symptom,"confidence":min(confidence or .75,.85),"source":"deterministic_message_fallback"})
...
class QwenInterpreter:
 ...
 def interpret(self,message,state):
  ...
  system="""Classify one printing-support turn. Return compact JSON only using schema enums. For troubleshooting, facts must include a symptom object. If product and symptom are present and the user asks what to validate, requested_action is retrieve. Do not answer technically. reasoning_summary max 12 words."""
  ...
  if r.finish_reason=="length":raise ValueError(f"Interpreter output was truncated at {r.usage.get('completion_tokens',self.max_tokens)} tokens")
