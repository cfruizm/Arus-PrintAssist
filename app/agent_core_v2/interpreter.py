...
def _normalize(raw,state):
 ...
 for x in r.get("entities") or []:
  ...
  kind=str(x.get("kind") or x.get("type") or "");name=str(x.get("canonical_name") or x.get("name") or x.get("matched_text") or "").strip()
 ...
 for x in r.get("facts") or []:
  if not isinstance(x,dict):continue
  category=aliases.get(str(x.get("type") or x.get("category") or x.get("key") or "technical_context"),str(x.get("type") or x.get("category") or x.get("key") or "technical_context"));value=str(x.get("value") or x.get("fact") or "").strip()
 ...
 # A confident technical intent is itself a technical request. Clarification is reserved for genuinely missing intent.
 confidence=float(r.get("confidence",0) or 0)
 if intent in TECHNICAL_INTENTS and confidence>=0.6 and act in {"clarification","unknown"}:
  act="technical_request";docs=True
