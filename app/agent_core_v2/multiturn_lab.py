...
def _checkpoint(t,e):
 b=t.get("state_before") or {};a=t.get("state_after") or {};d=t.get("decision") or {};answer=t.get("answer") or {};q=t.get("retrieval_query_trace") or {};topic=a.get("active_topic") or {};case=a.get("technical_case") or {};exp=e or {};checks={"intent":not exp.get("intent") or d.get("intent")==exp.get("intent"),"action":not exp.get("action") or d.get("action")==exp.get("action"),"products_present":all(x in _ids(topic.get("products")) for x in exp.get("products") or []),"products_absent":not set(exp.get("products_absent") or [])&set(_ids(topic.get("products"))),"symptom_present":not exp.get("symptom_contains") or str(exp["symptom_contains"]).casefold() in " ".join(case.get("symptoms") or []).casefold(),"scope_present":not exp.get("scope_contains") or str(exp["scope_contains"]).casefold() in str(case.get("affected_scope") or "").casefold(),"turn_incremented":int(a.get("turn_number",0))==int(b.get("turn_number",0))+1,"answer_present":bool(answer.get("text")),"contextual_query_when_retrieval":not d.get("requires_retrieval") or bool(q.get("contextual_query")),"production_unchanged":True};failed=[k for k,v in checks.items() if not v];return {"checks":checks,"failed_checks":failed,"status":"passed" if not failed else "failed"}
def run_multiturn_scenario(scenario,secrets,session_state):
 ...
 try:
  route=AdaptiveCostRouteController(int(secrets.get("AGENT_CORE_V2_INITIAL_CANDIDATES",3)),int(secrets.get("AGENT_CORE_V2_MAX_CANDIDATES",6)));evidence=SemanticEvidencePipeline(_retrieve,g,route.max_candidates,int(secrets.get("LLM_EVIDENCE_JUDGE_MAX_TOKENS",300)));engine=TurnEngine(QwenInterpreter(g,int(secrets.get("LLM_ORCHESTRATOR_MAX_TOKENS",220))),evidence,ResponseComposer(g,int(secrets.get("LLM_ANSWER_MAX_TOKENS",400))),EntityResolver(),route)
  ex=scenario.get("turn_expectations") or []
  for i,m in enumerate(scenario.get("messages") or []):
   t=engine.process_turn(str(m),state).to_dict();t["production_changed"]=False;t["multiturn_checkpoint"]=_checkpoint(t,ex[i] if i<len(ex) else {});turns.append(t)
  status="ok"
