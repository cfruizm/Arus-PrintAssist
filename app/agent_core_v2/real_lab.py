...
def _grade(t,s):
 d=t.get("decision") or {};e=t.get("evidence") or {};a=t.get("answer") or {};m=t.get("cost_route_metrics") or {};st=t.get("state_after") or {};exp=s.get("expected") or {};products=[x.get("canonical_id") for x in st.get("active_topic",{}).get("products",[])];case=st.get("technical_case") or {};symptoms=case.get("symptoms") or [];scope=case.get("affected_scope");approved={x.get("id") for x in e.get("citable",[])};requires=bool(d.get("requires_retrieval"));expected_scope=exp.get("affected_scope")
 checks={"intent":not exp.get("intent") or d.get("intent")==exp.get("intent"),"action":not exp.get("action") or d.get("action")==exp.get("action"),"entity":not exp.get("product_id") or exp.get("product_id") in products,"answer_present":bool(a.get("text")) or d.get("action") not in {"retrieve","out_of_scope"},"judge_only_when_needed":(requires and m.get("judge_calls",0)>=1) or (not requires and m.get("judge_calls",0)==0),"citation_integrity":set(a.get("citations") or []).issubset(approved),"response_not_truncated":a.get("finish_reason")!="length","troubleshooting_has_symptom":d.get("intent")!="troubleshooting" or bool(symptoms),"troubleshooting_is_diagnosing":d.get("intent")!="troubleshooting" or case.get("status")=="diagnosing","affected_scope_preserved":not expected_scope or str(expected_scope).casefold() in str(scope or "").casefold()}
