...
class ResponseComposer:
 ...
 def compose(self,message,decision,state,evidence):
  ...
  for item in approved:
   identity=str(item.get("url") or item.get("title") or item.get("id"));a=item.get("semantic_assessment") or {}
   claims=[str(x).strip() for x in (a.get("supported_claims") or []) if str(x).strip()]
   if identity in seen or not claims:continue
   seen.add(identity);sources.append({"id":item.get("id"),"title":item.get("title") or "Fuente documental","scope_relation":a.get("scope_relation"),"source_object":a.get("source_object"),"supported_claims":claims[:5],"conditions":(a.get("conditions") or [])[:3]})
