...
class TurnEngine:
 ...
 def process_turn(self,message,state):
  before=snapshot(state);p=self.interpreter.interpret(message,state);entities=self.resolver.resolve(message,p.entities);d=self.reconciler.reconcile(p,state,entities)
  # The conversation clock advances for every accepted turn, independent of domain mutation.
  state.turn_number+=1
  ...
  ev={};ans={};metrics={"retrieval_calls":0,"judge_calls":0,"answer_llm_calls":0,"expansion_calls":0,"calls_avoided":0,"route_plans":[]}
  ...
  if plan.deterministic_response is not None:ans={"mode":plan.route,"text":plan.deterministic_response,"citations":[],"knowledge_used":False}
   ...
   ev=self.evidence_engine.evaluate(message,d,state,plan.initial_candidates);metrics["retrieval_calls"]+=1;metrics["judge_calls"]+=1;after=self.route_controller.plan_after_judgment(d,ev,state);metrics["route_plans"].append(after.to_dict());metrics["calls_avoided"]+=after.estimated_calls_avoided
  ...
  result=TurnResult(message,before,p.__dict__,d.to_dict(),snapshot(state),directive,audit,ev,ans);data=result.to_dict();data["cost_route_metrics"]=metrics;data["interpreter_fallback_used"]=str(p.reasoning_summary).startswith("fallback:");return _TurnResultProxy(data)
