...
class TurnEngine:
 ...
 def process_turn(self,message,state):
  ...
  if plan.route in {"compose_lateral","compose_escalation","compose_cancel","compose_clarification","compose_out_of_scope"} and self.response_composer:
   ans=self.response_composer.compose_conversation(message,d,state,self.conversation_planner.build(d,state));metrics["answer_llm_calls"]+=0 if plan.route=="compose_out_of_scope" else 1
   ...
   if after.run_answer_llm and self.response_composer:ans=self.response_composer.compose(message,d,state,ev,self.conversation_planner.build(d,state,ev));metrics["answer_llm_calls"]+=1
  if not ans and self.response_composer:
   ans=self.response_composer.compose_conversation(message,d,state,self.conversation_planner.build(d,state,ev))
