...
class TurnEngine:
 ...
 def process_turn(self,message,state):
  ...
  if d.action=="record_case_detail":ans={"mode":"deterministic_case_detail_acknowledgement","text":build_case_detail_acknowledgement(state,d.facts),"citations":[],"knowledge_used":False}
  elif d.action=="record_attempt":ans={"mode":"deterministic_attempt_acknowledgement","text":"Entendido. Registré la acción realizada. ¿Qué resultado obtuviste?","citations":[],"knowledge_used":False}
  elif d.action=="record_attempt_result":ans={"mode":"deterministic_attempt_result_acknowledgement","text":"Entendido. Registré el resultado de la validación y lo tendré en cuenta para no repetir una acción que no resolvió el caso.","citations":[],"knowledge_used":False}
  elif plan.deterministic_response is not None:ans={"mode":plan.route,"text":plan.deterministic_response,"citations":[],"knowledge_used":False}
  elif plan.run_retrieval and self.evidence_engine:
   built=self.query_builder.build(message,d,state);qtrace=built.to_dict();ev=self.evidence_engine.evaluate(built.contextual_query,d,state,plan.initial_candidates);metrics["retrieval_calls"]+=1;metrics["judge_calls"]+=1;after=self.route_controller.plan_after_judgment(d,ev,state);metrics["route_plans"].append(after.to_dict());metrics["calls_avoided"]+=after.estimated_calls_avoided
   if after.allow_expansion and after.expansion_reason:
    expansion=self.route_controller.expansion_query(built.contextual_query,d,state,ev);qtrace["expansion_query"]=expansion;second=self.evidence_engine.evaluate(expansion,d,state,getattr(self.route_controller,"max_candidates",6)-plan.initial_candidates);metrics["retrieval_calls"]+=1;metrics["judge_calls"]+=1;metrics["expansion_calls"]+=1;ev=self.evidence_engine.merge_passes(ev,second);after=self.route_controller.plan_after_expansion(d,ev,state);metrics["route_plans"].append(after.to_dict());metrics["calls_avoided"]+=after.estimated_calls_avoided
  ...
  directive={"action":d.action,"intent":d.intent,"requires_retrieval":d.requires_retrieval,"citable_source_ids":[x["id"] for x in ev.get("citable",[])],"adaptive_route":metrics["route_plans"][-1]["route"] if metrics["route_plans"] else None};result=TurnResult(message,before,p.__dict__,d.to_dict(),snapshot(state),directive,audit,ev,ans);data=result.to_dict();data["cost_route_metrics"]=metrics;data["interpreter_fallback_used"]=str(p.reasoning_summary).startswith("fallback:");data["semantic_interpretation_trace"]=trace;data["retrieval_query_trace"]=qtrace;return _Proxy(data)
class _Proxy:
