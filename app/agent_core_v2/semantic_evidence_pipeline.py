...
class SemanticEvidencePipeline:
 ...
 def evaluate(self,query,decision,state,limit=None):
  raw=list(self.retriever(query,int(limit or self.max_candidates)) or []);candidates=[];seen=set()
  for index,item in enumerate(raw,1):
   ...
   if len(candidates)>=int(limit or self.max_candidates):break
  judged=self.judge.evaluate(query,decision.intent,decision.entities or state.active_topic.products,candidates)
  if judged.get("ok"):result=merge_judgment(candidates,judged);result["unassessed"]=[]
  else:
   unassessed=[]
   for item in candidates:
    copy=dict(item);copy["semantic_assessment"]={"applicability":"unassessed","reason":"No evaluada por fallo técnico del juez.","supported_claims":[],"conditions":[]};copy["eligible"]=False;copy["citable"]=False;unassessed.append(copy)
